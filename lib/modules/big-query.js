...
function insert(data) {
  const bigquery = BigQuery({
    projectId: process.env.PROJECT_ID || data.config.projectId,
    keyFilename: process.env.KEYFILE || data.config.keyfile
  });

  return createDatasetifDoesntExist(data.config.dataset, bigquery)
    .then((results) => {
      return createTableIfDoesntExist(results, data.config.table, data.config.schema);
    })
    .then((table) => {
