...
const BigQuery = require('@google-cloud/bigquery'),
  ...
  DATASET = 'clay_sites'; // Sets a default BigQuery dataset for now
...
function insert(data) {
  const bigquery = BigQuery({
    ...
    keyFilename: data.config.keyfile
  });

  return createDatasetifDoesntExist(DATASET, bigquery)
    .then((results) => {
      return createTableIfDoesntExist(results, 'content_classification', data.config.schema);
    })
    .then((table) => {
      let rawData = _omit(data, ['config']); // Omit the config obj from insert

      return table.insert(rawData)
        .then((results) => {
