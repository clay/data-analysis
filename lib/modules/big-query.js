 ...
 * @returns {Promise}
 */
function insert(data, argv) {
  console.log('what is our data for BigQuery', data);
  const bigquery = BigQuery({
    projectId: 'nymag-analytics-157315' || data.config.projectId,
    keyFilename: './keyfile.json' || data.config.keyfile 
  });

  return createDatasetifDoesntExist(DATASET, bigquery)
    .then((results) => {
      return createTableIfDoesntExist(results, 'vindicated', data.config.schema);
    })
    .then((table) => {
